...
const loginUser_post = asyncHandler(async (req, res, next) => {
    ...
    return successResponse(res, 'Login was successfull', { accessToken, roles: user.role });
})
// logging out user
...
const refresh_post = asyncHandler(async (req, res, next) => {
    const refreshToken = req.cookies.jwt;
    if (!refreshToken)
        return next(new ErrorResponse('Unauthorized, No refresh token provided', 401));
    const accessToken = await authService.refresh(refreshToken);
    return successResponse(res, 'Tokens refreshed successfully', { accessToken })
})
//verify email
