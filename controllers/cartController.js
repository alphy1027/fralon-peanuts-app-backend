...
const removeFromCart_delete = asyncHandler(async (req, res, next) => {
  ...
  if (!cart) return next(new ErrorResponse("No cart found with this user ID", 404));
  return successResponse(res, "Product removed from cart successfully", { updatedCart: cart });
});

const updateQuantity_patch = asyncHandler(async (req, res, next) => {
  const clientId = req.user.userId;
  const { productId, quantity } = req.body;
  validateId(productId, next);
  const cart = await cartService.updateItemQuantity(clientId, productId, quantity);
  if (!cart) return next(new ErrorResponse("Failed to add item quantity", 400));
  return successResponse(res, "Product quantity updated successfully", { cart: updatedCart });
});
