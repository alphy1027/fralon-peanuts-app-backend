const { createAccessToken, verifyToken, createRefreshToken, generateCryptoToken } = require('../middleware/generateToken');
...
class AuthService {
    ...
    async resetPassword(password, confirmPassword, resetPasswordToken) {
        if (password !== confirmPassword)
            throw new ErrorResponse('Passwords you provided do not match', 400);
        ...
        if (!user)
            throw new ErrorResponse('Invalid or Expired reset password link', 404);
