...
class OrderService {
    ...
    async createOrder(orderDetails) {
        ...
        try {
            await session.withTransaction(async () => {
                const order = await this.Order.create([{ ...orderDetails }], { session });
                newOrder = order[0];
                if (!newOrder)
                    throw new ErrorResponse('Failed to create order', 400);
                const clientOrders = await this.addOrderToClient(newOrder.client, newOrder._id, session);
                if (!clientOrders)
                    throw new ErrorResponse('Failed to add order to client document', 400);
    ...
    async updateOrder(orderId, orderDetails) {
        ...
        return await this.Order.findByIdAndUpdate(orderId,
            { $set: { ...orderDetails } },
