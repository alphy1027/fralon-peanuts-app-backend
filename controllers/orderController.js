...
const getRevenueTrends_get = asyncHandler(async (req, res, next) => {
    ...
    if (!revenueTrends)
        return next(new ErrorResponse('Could not find the specified revenue trends', 404))
...
const updateOrderStatus_patch = asyncHandler(async (req, res, next) => {
    ...
    if (!updatedOrder)
        return next(new ErrorResponse('Order status could not be updated', 404));
...
const placeOrder_post = asyncHandler(async (req, res, next) => {
    ...
    if (!newOrder)
        return next(new ErrorResponse('New order not found', 404))
...
const cancelOrder_patch = asyncHandler(async (req, res, next) => {
    ...
    if (!cancelledOrder)
        return next(new ErrorResponse('No order matches that ID', 404))
