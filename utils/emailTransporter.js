...
const sendVerificationEmail = (email, token, username) => {
    const verifyEmailUrl = `${process.env.FRONTEND_URL}/verify-email/${token}`;
    ...
    return new Promise((resolve, reject) => {
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if (err) {
                reject({ status: 500, message: 'Failed to send verification Email, check your connection', error: err.message });
            }
...
const sendForgotPasswordEmail = (email, token, username) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if (err) {
                reject({ status: 500, message: 'Failed to send reset password Email, check your connection', error: err.message });
            }
...
const sendContactEmail = (email, name, message) => {
    const mailOptions = {
        ...
        text: message,
        html:
            `<p>You have a new contact form submission from ${name}</p>
