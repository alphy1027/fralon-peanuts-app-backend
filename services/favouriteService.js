...
class FavouriteService {
    ...
    async getFavouriteProducts(clientId) {
        ...
        if (!client)
            throw new ErrorResponse('Unknown user can not request for favourite products', 404)
        return client.favouriteProducts
    }

    async addFavouriteProduct(clientId, productId) {
        const updatedClient = await this.Client.findByIdAndUpdate(clientId,
            { $addToSet: { favouriteProducts: productId } },
            { new: true }
        )
        if (!updatedClient)
            throw new ErrorResponse('Unknown user can not add products to favourites', 404);
    ...
    async removeFavouriteProduct(clientId, productId) {
        ...
        if (!updatedClient)
            throw new ErrorResponse('Unknown user can not remove products from Favourites', 404)

        if (updatedClient.favouriteProducts.includes(productId.toString()))
            throw new ErrorResponse('Product has not been added to Favourites', 400)
        return updatedClient.favouriteProducts;
    }
